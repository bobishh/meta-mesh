...
- Match `8b415d4` and standalone lighthouse `c450b1a` now dial every endpoint from the verified peer catalog, including editors when the invitation issuer is offline. The Rust `MeshRuntimeState` schedules bounded reconnects; the local route advertisement increments once per node start rather than on every failed dial. The targeted Match Playwright owner-offline/editor-write/native-restart scenario passed after this change; standalone `cargo check --locked` passed. This is local evidence: the deployed Hetzner image still predates these commits, and no owner-issued production invite has paired the service.
- MetaMesh `7d84421` deletes an unused TypeScript `AutomergeSyncScheduler` and its orphaned test; the live `syncAutomergeDocumentToDevice` path already takes route, retry, and ACK decisions from Rust flows, with TypeScript executing browser transport and storage callbacks. MetaMesh TypeScript check and focused Automerge tests passed. Twang `38f6dd9` and Match `4c3cfe0` pin this same core; Twang browser build and 24 engine tests passed, Match browser build passed. Standalone lighthouse `e286b40` pins the updated Match dependency. Remaining Match/Twang orchestration in 7.3–7.5 is not claimed complete by these checks.
