...
impl BrowserRuntime {
    ...
    pub(crate) fn close_node(
        ...
        // A completed WebRTC dial removes its bootstrap writer before the
        // application connection is later closed. Terminal signals are no
        // longer deliverable in that case, but must not prevent node cleanup.
        let _ = send_outbound_signals(&self.bootstrap, &result.outbound_signals);
        self.close();
        Ok(result)
    }
