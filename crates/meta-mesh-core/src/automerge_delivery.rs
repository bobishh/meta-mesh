...
pub fn decode_device_sync_request(
    raw: Value,
    expected_remote_device_id: &str,
) -> Result<DeviceSyncInbound, String> {
    let request: IncomingDeviceSyncRequest = serde_json::from_value(raw)
        .map_err(|_| "Invalid Automerge device sync request".to_string())?;
    if request.kind != "mesh-automerge-device-sync"
        || request.version != 1
        || request.batch_id.is_empty()
    {
        return Err("Invalid Automerge device sync request".into());
    }
    let frame = request.frame.decode()?;
    if frame.version != 1
        || frame.scope_id.is_empty()
        || frame.document_id.is_empty()
        || frame.from_device_id.is_empty()
        || frame.to_device_id.is_empty()
    {
    ...
    if request.batch_id != hash {
        return Err("Automerge sync batch does not match frame".into());
    }
    Ok(DeviceSyncInbound {
        batch: DeviceBatch {
            protocol_version: 1,
...
pub fn encode_device_sync_response(
    batch_id: String,
    ack: Value,
    frame: Option<AutomergeSyncFrame>,
) -> Result<Value, String> {
    if batch_id.is_empty() || ack.is_null() {
        return Err("Invalid Automerge device sync response".into());
    }
    let frame = frame.map(|frame| DeviceSyncWireFrame {
        version: frame.version,
        scope_id: frame.scope_id,
        document_id: frame.document_id,
        from_device_id: frame.from_device_id,
        to_device_id: frame.to_device_id,
        message: URL_SAFE_NO_PAD.encode(frame.message),
        proof: frame.proof,
    });
    let mut response = serde_json::json!({ "kind": "mesh-automerge-device-sync-response",
        "version": 1, "batchId": batch_id, "ack": ack });
    if let Some(frame) = frame {
        response["frame"] = serde_json::to_value(frame)
...
impl AutomergeDeviceSyncFlow {
    pub fn new(
        ...
        Ok(Self {
            ...
            maximum_rounds,
            rounds: 0,
            selected_route_instance_id: None,
            route_instance_ids: Vec::new(),
    ...
    pub fn next(&mut self, frame: Option<AutomergeSyncFrame>) -> Result<DeviceSyncStep, String> {
        ...
        let Some(frame) = frame else {
            return Ok(DeviceSyncStep::Converged(DeviceSyncConverged {
                rounds: self.rounds,
                route_instance_ids: self.route_instance_ids.clone(),
            }));
        };
        ...
        if frame.version != 1
            || frame.document_id != self.document_id
            || frame.to_device_id != self.target_device_id
            || frame.scope_id.is_empty()
            || frame.from_device_id.is_empty()
        {
            return Err("Invalid Automerge device sync frame".into());
        ...
        Ok(DeviceSyncStep::Round(DeviceSyncRound {
            round: self.rounds + 1,
            batch,
            request,
    ...
    pub fn complete_round(
        ...
        if route_instance_id.is_empty() {
            return Err("Invalid Automerge sync route".into());
        }
        let response: IncomingDeviceSyncResponse = serde_json::from_value(raw)
            .map_err(|_| "Invalid Automerge device sync response".to_string())?;
        self.check_response(&response)?;
    ...
    fn check_response(&self, response: &IncomingDeviceSyncResponse) -> Result<(), String> {
        if response.kind != "mesh-automerge-device-sync-response"
            || response.version != 1
            || self.pending_batch_id.as_deref() != Some(response.batch_id.as_str())
            || response.ack.is_null()
        {
            return Err("Invalid Automerge device sync response".into());
        }
        if let Some(frame) = &response.frame {
            if frame.version != 1
                || frame.scope_id != self.pending_scope_id.as_deref().unwrap_or_default()
                || frame.document_id != self.document_id
                || frame.from_device_id != self.target_device_id
...
mod tests {
    ...
    fn failed_route_cannot_advance_round_and_success_reuses_route_until_convergence() {
        ...
        let frame = AutomergeSyncFrame {
            version: 1,
            scope_id: "chat".into(),
            document_id: "chat".into(),
            from_device_id: "alice".into(),
            to_device_id: "bob".into(),
            message: vec![1, 2],
            proof: None,
        };
        let DeviceSyncStep::Round(first) = flow.next(Some(frame.clone())).unwrap() else {
            panic!()
        };
        ...
        assert!(
            flow.complete_round("route-a".into(), accepted)
                .unwrap()
                .is_none()
        );
        let DeviceSyncStep::Round(second) = flow.next(Some(frame)).unwrap() else {
            panic!()
        };
        assert_eq!(
            second.selected_route_instance_id.as_deref(),
            Some("route-a")
        );
        ...
        assert!(
            flow.next(Some(AutomergeSyncFrame {
                version: 1,
                scope_id: "chat".into(),
                document_id: "chat".into(),
                from_device_id: "alice".into(),
        ...
        let DeviceSyncStep::Converged(done) = flow.next(None).unwrap() else {
            panic!()
        };
