...
use crate::{
    ...
    WorkspaceAuthority, WorkspaceDeviceRevocation, public_key_id, verify_device_certificate_chain,
...
impl WorkspaceGrantPayload {
    pub fn effective_access_epoch(&self) -> u64 {
        self.access_epoch.unwrap_or(1)
    }
}
...
fn admit_with_context(
    ...
    let (role, grant_epoch) = context.role_for(
        &payload.person_id,
        &payload.device_id,
        ...
        .filter_map(|hash| {
            if !context.revocation_allows(&payload.person_id, grant_epoch, hash)
                ...
                || (role == WorkspaceRole::Owner
                    ...
                    && !context
                        .historical_hashes
                        .get(&payload.person_id)
                        .is_some_and(|hashes| hashes.contains(hash)))
            {
                None
            } else {
                Some(AuthorizedWorkspaceChange {
                    hash: hash.clone(),
                    role,
                })
            }
...
pub(crate) struct ValidatedWorkspaceWriteAuthorizationContext {
    ...
    revoked_devices:
        std::collections::HashMap<(String, String), Vec<std::collections::HashSet<String>>>,
...
impl ValidatedWorkspaceWriteAuthorizationContext {
    pub(crate) fn from_snapshot(
        ...
        if raw.workspace_id.is_empty()
            || raw.document.is_empty()
            || raw.ownership_transfers.len() > 32
            || raw.succession_claims.len() > 32
            || raw.revocations.len() > 512
            || raw.departures.len() > 512
        ...
        for evidence in &raw.device_revocations {
            ...
            .is_ok();
            let signed_by_subject = !signed_by_owner
                && crate::authority::verify_workspace_device_revocation(
                    &evidence.record,
                    &raw.workspace_id,
                    &current_owner.person_id,
                    &evidence.signer,
    ...
    ) -> Result<(WorkspaceRole, Option<u64>), String> {
        if person_id == self.current_owner.person_id {
            return Ok((WorkspaceRole::Owner, None));
        }
        let verified_grant = self
            .current_owner_and_history()
            .into_iter()
            .find_map(|authority| {
                let grant = grant?;
    ...
    fn boundaries_allow(
        &self,
        boundaries: Option<&Vec<RevocationBoundary>>,
        grant_epoch: Option<u64>,
        hash: &str,
    ) -> bool {
        boundaries.is_none_or(|boundaries| {
            boundaries.iter().all(|boundary| {
                grant_epoch.is_some_and(|epoch| epoch > boundary.epoch)
                    || boundary.hashes.contains(hash)
            })
        })
    }
...
pub(crate) fn validate_authority(authority: &WorkspaceAuthority) -> Result<(), String> {
    if authority.person_id.is_empty()
        || authority.public_key.is_empty()
...
impl OwnershipTransition {
    fn from_owner(&self) -> &str {
        match self {
            Self::Transfer(record) => &record.payload.from_owner_person_id,
            Self::Succession(record) => &record.payload.from_owner_person_id,
        }
    }
    fn epoch(&self) -> u64 {
        match self {
            Self::Transfer(record) => record.payload.epoch,
            Self::Succession(record) => record.payload.epoch,
        }
    }
    fn heads(&self) -> &[String] {
        match self {
            Self::Transfer(record) => &record.payload.workspace_heads,
            Self::Succession(record) => &record.payload.workspace_heads,
        }
    }
    fn next_authority(&self) -> WorkspaceAuthority {
        match self {
            Self::Transfer(record) => WorkspaceAuthority {
            ...
            Self::Succession(record) => WorkspaceAuthority {
                person_id: record.payload.to_owner_person_id.clone(),
                public_key: record.payload.to_owner_public_key.clone(),
                certificates: record.payload.to_owner_certificates.clone(),
            },
...
pub(crate) fn verified_ownership_chain(
    raw: &WorkspaceWriteAuthorizationSnapshot,
    now_ms: i128,
    ...
    String,
> {
    let by_hash = if require_document_heads {
        document_hashes(&raw.document)?
    } else {
    ...
    let mut transitions = raw
        ...
        .iter()
        .cloned()
        .map(OwnershipTransition::Transfer)
        .chain(
            raw.succession_claims
                .iter()
                .cloned()
                .map(OwnershipTransition::Succession),
        )
        .collect::<Vec<_>>();
    ...
    while !transitions.is_empty() {
        let next_epoch = transitions
            .iter()
            .filter(|record| record.from_owner() == current.person_id && record.epoch() > epoch)
            .map(OwnershipTransition::epoch)
            .min()
            .ok_or_else(|| "Missing workspace ownership transition".to_string())?;
        let candidates = transitions
            .iter()
            .enumerate()
            .filter(|(_, record)| {
                record.from_owner() == current.person_id && record.epoch() == next_epoch
            })
            .map(|(index, _)| index)
            .collect::<Vec<_>>();
        if candidates.len() > 1 {
            return Err("Conflicting workspace ownership transitions".to_string());
        }
        ...
        match &transition {
            OwnershipTransition::Transfer(record) => {
                crate::authority::verify_workspace_ownership_transfer(
                    record,
                    &raw.workspace_id,
                    &current,
                    epoch,
            ...
            OwnershipTransition::Succession(record) => {
                crate::authority::verify_workspace_succession_claim(
                    record,
                    &raw.workspace_id,
                    &current,
                    epoch,
        ...
        let allowed = if require_document_heads {
            ancestry(transition.heads(), &by_hash)?
        } else {
            std::collections::HashSet::new()
        };
        historical_hashes
            .entry(current.person_id.clone())
...
fn ancestry(
    heads: &[String],
    by_hash: &std::collections::HashMap<String, Vec<String>>,
) -> Result<std::collections::HashSet<String>, String> {
    let mut allowed = std::collections::HashSet::new();
    let mut pending = heads.to_vec();
    while let Some(hash) = pending.pop() {
        if !allowed.insert(hash.clone()) {
            continue;
        }
        let dependencies = by_hash.get(&hash).ok_or_else(|| {
            "Workspace ownership boundary is missing from the document".to_string()
        })?;
    ...
    Ok(document
        ...
        .map(|change| {
            (
                ...
                change.deps().iter().map(ToString::to_string).collect(),
            )
        })
        .collect())
}

fn verified_revocation_boundaries(
    raw: &WorkspaceWriteAuthorizationSnapshot,
    ...
    for record in &raw.revocations {
        let authority = authorities
            .iter()
            .find(|authority| authority.person_id == record.payload.owner_person_id)
            .ok_or_else(|| "Workspace revocation has an unknown owner".to_string())?;
        crate::authority::verify_workspace_revocation(
            record,
            &raw.workspace_id,
            authority,
            now_ms,
        ...
        boundaries
            .entry(record.payload.person_id.clone())
            .or_default()
            .push(RevocationBoundary {
...
fn verified_departure_boundaries(
    ...
    for evidence in &raw.departures {
        ...
        boundaries
            ...
            .or_default()
            .push(RevocationBoundary { epoch, hashes });
    }
    Ok(boundaries)
}
...
mod tests {
    ...
    fn transfer(
        ...
        owner_seed: &[u8; 32],
        owner_device_seed: &[u8; 32],
        owner_device_id: &str,
        next: &WorkspaceAuthority,
        heads: Vec<String>,
        epoch: u64,
    ) -> crate::WorkspaceOwnershipTransfer {
        let to_owner = WorkspaceGrantPayload {
            kind: "workspace-grant".into(),
            version: 1,
            grant_id: "next-owner".into(),
            workspace_id: "workspace".into(),
        ...
        let payload = crate::WorkspaceOwnershipTransferPayload {
            kind: "workspace-ownership-transfer".into(),
            version: 1,
            workspace_id: "workspace".into(),
            from_owner_person_id: owner.person_id.clone(),
            to_owner_person_id: next.person_id.clone(),
            to_owner_public_key: next.public_key.clone(),
            to_owner_certificates: next.certificates.clone(),
            to_owner_grant,
            former_owner_grant,
    ...
    fn revocation(
        ...
        heads: Vec<String>,
        epoch: u64,
    ) -> crate::WorkspaceRevocation {
        let payload = crate::WorkspaceRevocationPayload {
    ...
    fn device_revocation(
        ...
        device_id: &str,
        heads: Vec<String>,
    ) -> WorkspaceDeviceRevocationEvidence {
        let payload = crate::WorkspaceDeviceRevocationPayload {
    ...
    fn departure(
        person: &WorkspaceAuthority,
        device_seed: &[u8; 32],
        device_id: &str,
        heads: Vec<String>,
        epoch: u64,
    ) -> WorkspaceDepartureEvidence {
        let payload = crate::authority::WorkspaceDeparturePayload {
            kind: "workspace-departure".into(),
            version: 1,
            workspace_id: "workspace".into(),
            person_id: person.person_id.clone(),
            access_epoch: epoch,
            workspace_heads: heads,
            left_at: "1970-01-01T00:00:00.000Z".into(),
        };
        let signed = sign_json_envelope(
            device_seed,
            serde_json::to_value(&payload).unwrap(),
        ...
        .unwrap();
        WorkspaceDepartureEvidence {
            record: crate::authority::WorkspaceDeparture {
                payload,
                signer_key_id: signed.signer_key_id,
                signature: signed.signature,
            },
    ...
    fn batch_admission_rejects_everything_when_one_signature_is_invalid() {
        ...
        assert!(
            admit_workspace_change_authorizations(&[first, second], &context, &needed, 0).is_err()
        );
    ...
    fn admits_a_current_owner_change_authorization() {
        ...
        let incoming = change_authorization(
            &owner,
            &device_seed,
            &device_id,
            certificates,
        ...
        assert_eq!(
            admit_workspace_change_authorization(
                &incoming,
                &snapshot(owner),
                &["current".into()],
                0
    ...
    fn revocation_boundary_keeps_old_history_denies_new_hashes_and_allows_newer_grant() {
        ...
        let make_incoming = |access_epoch| {
            let grant_payload = WorkspaceGrantPayload {
                kind: "workspace-grant".into(),
                version: 1,
                grant_id: format!("editor-{access_epoch}"),
                workspace_id: "workspace".into(),
                person_id: editor.person_id.clone(),
                role: WorkspaceRole::Editor,
        ...
        context
            .revocations
            .push(revocation(&owner, &[31; 32], &editor.person_id, heads, 2));

        let stale = admit_workspace_change_authorization(
            &make_incoming(1),
            &context,
        ...
        let renewed = admit_workspace_change_authorization(
            &make_incoming(3),
            &context,
            &[before.clone(), after.clone()],
            0,
        ...
        forged.grant = Some(signed_grant(
            &[99; 32],
            &owner.person_id,
            &forged.grant.unwrap().payload,
        ));
    ...
    fn departure_boundary_preserves_old_history_and_denies_new_changes() {
        ...
        context
            .departures
            .push(departure(&editor, &device_seed, &device_id, heads, 1));
        assert_eq!(
            admit_workspace_change_authorization(&incoming, &context, &[before.clone(), after], 0)
                .unwrap(),
            vec![AuthorizedWorkspaceChange {
                hash: before,
                role: WorkspaceRole::Editor
    ...
    fn device_revocation_preserves_boundary_history_and_denies_later_device_changes() {
        ...
        let (editor, editor_device_seed, editor_device_id, certificates) =
            authority([49; 32], [50; 32]);
        let mut document = automerge::AutoCommit::new();
        document.put(automerge::ROOT, "before", "value").unwrap();
        document.commit();
        let before = document.get_heads()[0].to_string();
        ...
        let mut context = snapshot(owner.clone());
        context.document = document.save();
        context.device_revocations.push(device_revocation(
            &owner,
            &owner_device_seed,
            &owner_device_id,
            &editor.person_id,
    ...
    fn rejects_revocation_boundary_missing_from_document() {
        ...
        let mut context = snapshot(owner.clone());
        context.document = document.save();
        context.revocations.push(revocation(
            &owner,
            &[35; 32],
            &editor.person_id,
            vec!["missing-head".into()],
            2,
        ));
        assert_eq!(
            admit_workspace_change_authorization(&incoming, &context, &[hash], 0).unwrap_err(),
    ...
    fn limits_a_historical_owner_to_its_transfer_boundary() {
        ...
        context.ownership_transfers.push(transfer(
            &owner,
            &[11; 32],
            &owner_device_seed,
            &owner_device_id,
    ...
    fn rejects_conflicting_or_missing_ownership_boundaries() {
        ...
        assert_eq!(
            admit_workspace_change_authorization(&incoming, &context, &["change".into()], 0)
                .unwrap_err(),
            "Conflicting workspace ownership transitions"
        );
        context.ownership_transfers = vec![transfer(
            &owner,
            &[17; 32],
            &owner_device_seed,
            &owner_device_id,
    ...
    fn admits_a_monotonic_transfer_epoch_after_access_epoch_changes() {
        ...
        assert_eq!(
            admit_workspace_change_authorization(&incoming, &context, &[boundary.clone()], 0)
                .unwrap(),
            vec![AuthorizedWorkspaceChange {
                hash: boundary,
                role: WorkspaceRole::Owner
    ...
    fn verified_transition_selects_signed_owner_after_access_epoch_gap() {
        ...
        let records = vec![
            serde_json::json!({"payload": {"fromOwnerPersonId": "other", "epoch": 9}}),
            serde_json::to_value(&record).unwrap(),
        ];
        let plan = crate::next_verified_ownership_transition(
            &records,
            "workspace",
            &owner,
            1,
    ...
    fn verified_transition_stalls_conflicts_and_rejects_forgery_or_revocation() {
        ...
        let records = vec![
            serde_json::to_value(&carol_transfer).unwrap(),
            serde_json::to_value(&bob_transfer).unwrap(),
        ];
        let plan = crate::next_verified_ownership_transition(
            &records,
            "workspace",
        ...
        assert!(
            crate::next_verified_ownership_transition(
        ...
        assert_eq!(
            crate::next_verified_ownership_transition(
                ...
                &owner,
                1,
                &std::collections::HashSet::from([bob.person_id]),
                0
            )
            .unwrap_err(),
            "New owner access is revoked"
        );
    }
    ...
    fn rejects_an_absent_authority_context() {
        ...
        let context = WorkspaceWriteAuthorizationSnapshot {
    ...
    fn legacy_grant_without_access_epoch_keeps_its_signed_json_shape() {
        ...
        assert_eq!(
            serde_json::to_value(&decoded).unwrap()["payload"].get("accessEpoch"),
            None
        );
        assert_eq!(
            verify_workspace_grant(
